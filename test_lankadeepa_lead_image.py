...
for a in soup.select("a[href]"):
    href = a.get("href")
    if ("/latest_news/" in href or "/news/" in href) and "-" in href:
...
def extract_lankadeepa_lead_image(soup: BeautifulSoup, page_url: str) -> str | None:
    ...
    for selector in ["div.col-md-8 p img", "div.col-md-8 img", "div.article-body img", "div.post-content img", "article img"]:
        for img in soup.select(selector):
            src = img.get("src") or img.get("data-src")
            if src and isinstance(src, str):
                # Ignore static generic logos/icons/ads
                if any(ignored in src for str_val in ["logo", "icon", "image_8df7de9e07", "hit-ad"] for ignored in [str_val]):
                    continue
                return src
    ...
    if og:
        ...
        if isinstance(content, str) and "image_8df7de9e07" not in content and "logo" not in content.lower():
            return content
