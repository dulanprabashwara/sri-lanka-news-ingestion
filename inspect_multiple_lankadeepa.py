...
for a in soup.select("a[href]"):
    href = a.get("href")
    if ("/latest_news/" in href or "/news/" in href) and href not in links:
        links.append(href)
    if len(links) >= 5:
        break
...
for link in links:
    ...
    for img in s.select(
        "div.article-content img, div.post-content img, div.news-detail img, div.single-news img, article img, .image-container img, img"
    ):
