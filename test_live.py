...
def test_lankadeepa():
    ...
    print('JSON-LD:', [s.string[:50] for s in soup4.find_all('script', type='application/ld+json') if s.string])
    body_tags = []
    for c in soup.find_all(class_=lambda x: x and ('content' in x or 'post' in x)):
        if len(c.get_text(strip=True)) > 200:
            body_tags.append('.'.join(c.get('class')))
    print('Lankadeepa body candidates:', body_tags)
    for article in soup4.find_all('article'):
        print('Has article tag')
