...
for article_id, url in URLS.items():
    ...
    for selector in [
        "div.post-content", "div.entry-content", "article",
        "div.news-content", "div.story-body", "div.article-body",
        "div.article-text", "div.news-body", "div.content-body",
        "div.col-md-12",
    ]:
    ...
    for p in all_p:
        ...
        if key not in seen_parents:
            ...
            if text:
                sys.stdout.buffer.write(f"  <p> in <{parent_tag} class='{parent_cls}'>: {text}\n".encode('utf-8'))
