...
def analyze(url):
    ...
    print(f"og:url tag: {og_url_text}".encode('utf-8') if og_url_text else b"og:url tag: None")
    
    h1 = soup.find('h1')
    print(f"Title: {h1.get_text().strip() if h1 else None}".encode('utf-8'))
    
    body = soup.select_one('article p, div.post-content p, div.entry-content p')
    if body:
        print(f"Body start: {body.get_text().strip()[:50]}...".encode('utf-8'))
